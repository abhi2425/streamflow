  ...
  <div className='reviews flex-x-between margin-t-s'>
    <div className='flex-row'>
      <i className='icon m-right-s' id='upVote' onClick={() => voteHandler(true)}>
        <FaThumbsUp
          id='upVote'
          fill={state.upVote_active ? 'var(--color-profile-1)' : 'var(--color-dark-grey)'}
        />
      </i>
      <i id='downVote' className='icon m-right-s' onClick={() => voteHandler(false)}>
        <FaThumbsDown
          id='downVote'
          fill={state.downVote_active ? 'var(--color-profile-1)' : 'var(--color-dark-grey)'}
        />
      </i>
      <i className='icon icon-grey m-right-s' onClick={() => setShowComments((prev) => !prev)}>
        <FaRegComment fill={showComments ? 'var(--color-profile-1)' : 'var(--color-dark-grey)'} />
      </i>
    </div>
    <p>
